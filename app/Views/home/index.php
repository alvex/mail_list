...
<body>
    <div class="container">
        ...
        <button id="exportCsvBtn" class="fa fa-file-text-o" style="margin-top: 10px;"> Exportar CSV (User y
            Email)</button>
    ...
    <script>
        ...
        async function loadLatestSummary() {
            try {
                const res = await fetch('<?php echo BASE_PATH; ?>/api/latest');
                if (res.status === 403) {
        ...
        document.getElementById('user-form').addEventListener('submit', async (event) => {
            ...
            // Removed customers_id as it was unused in logic but present in form submit
            const customers_id = 0;
            try {
                const response = await fetch('<?php echo BASE_PATH; ?>/api/fetch', {
                    method: 'POST',
